...
class MediaDialog(Gtk.Dialog):
    def __init__(self, parent, unused):
        ...
        self.window.set_properties(self, title="Media Loader")
        ...
        tpak_frame = Gtk.Frame(label="Transfer Pak options")
        self.set_margin(tpak_frame, 5, 5, 5, 5)
        n64dd_frame = Gtk.Frame(label="64DD options")
        self.set_margin(n64dd_frame, 5, 5, 5, 5)

        tpak_grid = Gtk.Grid()

        self.parent.m64p_wrapper.ConfigOpenSection("Transferpak")

        player1_rom_label = Gtk.Label(label="GB game (1):")
        ...
        player1_ram_label = Gtk.Label(label="GB save (1):")
        ...
        player2_rom_label = Gtk.Label(label="GB game (2):")
        ...
        player2_ram_label = Gtk.Label(label="GB save (2):")
        ...
        player3_rom_label = Gtk.Label(label="GB game (3):")
        ...
        player3_ram_label = Gtk.Label(label="GB save (3):")
        ...
        player4_rom_label = Gtk.Label(label="GB game (4):")
        ...
        player4_ram_label = Gtk.Label(label="GB save (4):")
        ...
        tpak_frame.set_child(tpak_grid)

        n64dd_grid = Gtk.Grid()

        self.parent.m64p_wrapper.ConfigOpenSection("64DD")

        ipl_rom_label = Gtk.Label(label="IPL ROM:")
        ...
        disk_rom_label = Gtk.Label(label="Disk ROM:")
        ...
        n64dd_frame.set_child(n64dd_grid)

        media_box = self.window.get_content_area()
        media_box.set_property("orientation", Gtk.Orientation.VERTICAL)
        media_box.append(tpak_frame)
        media_box.append(n64dd_frame)
    ...
    def on_search_file_path(self, widget, entry, mem):
        dialog = w_dialog.FileChooserDialog(self.window, "gameboy", mem)
    ...
    def on_search_64dd_path(self, widget, entry, mem):
        dialog = w_dialog.FileChooserDialog(self.window, "64dd", mem)
        self.filename = dialog.path
        if self.filename != None:
            self.is_changed = True
            self.apply_button.set_sensitive(True)
